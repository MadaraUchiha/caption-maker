...
let fetchImage = Promise.coroutine(function*(uri, location = "./target") {
    yield delAsync(`${location}/*`);
...
let addTextGif = Promise.coroutine(function*(location, extension, topText, bottomText) {
    let {width, height} = sizeOf(`${location}/input.${extension}`);
    let {topPtSize, bottomPtSize} = getTextSizes(width, height, topText, bottomText);
    yield fs.mkdirAsync(`${location}/images`);
    let [stdout, stderr] = yield execAsync(`identify -verbose ${location}/input.${extension} | grep -i delay`);
    let animationDelay = stdout.split('\n')[0].replace(/.+Delay: /, "");
    yield execAsync(`convert -coalesce ${location}/input.${extension} ${location}/images/input_%06d.${extension}`);
    yield execAsync(`mogrify -font "Impact" -fill white -stroke black \
                     -pointsize ${topPtSize} -gravity north -draw "text 0,20 '${topText}'" \
                     -pointsize ${bottomPtSize} -gravity south -draw "text 0,20 '${bottomText}'" \
                     ${location}/images/*`);
